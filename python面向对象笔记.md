...
> 实现：传入(非父类和子类)函数的参数为对象，而对象属于不同类，就会产生不同的执行效果
...
4.单例--让**类**创建的对象，在系统中**只有唯一一个的实例**

- 定义一个**类属性**，初始值是`None`，用于记录**单例对象的引用**
- 重写`__new__`方法
  - 如果**类属性**`is Nnoe`，调用父类方法分配空间，并在类属性中记录结果
  - 返回**类属性**中记录的**对象引用**
...
- 再次**自动**调用`__init__`方法时，**初始化动作就不会被执行**

### 异常

1.异常的概念

- 程序在运行时遇到错误，**会停止程序的执行，并且提示一些错误信息**，这就是**异常**
- **程序停止执行并提示错误信息**称为**抛出异常**
...
- 当函数/方法执行**出现异常**，会将**异常**传递给函数/方法**调用**的一方
...
- 创建一个`Exception`的**对象**
- 使用`raise`**关键字**抛出异常对象
- 在调用的函数中使用`except Exception as result` 捕获异常

### 模块

1.概念

- 每一个python源文件代码都是一个模块
- **模块名**同样是一个**标识符**，需要符合标识符的命名规则
- 在模块中定义的**全局变量、函数、类**都是提供给外界直接使用的工具

2.通过`模块名.`使用**模块提供的工具--全局变量、函数、类**

> 如果模块名字太长，可以使用`as`指定模块的名称，模块别名应使用大驼峰命名法

3.`from...import`导入

- 如果希望**从某一个模块**中，导入**部分**工具
...
> 如果两个模块，存在同名的函数，那么**后导入模块的函数**，会**覆盖掉先导入的函数**,一旦发现冲突，可以使用`as`关键字，给其中一个工具起别名

4.`from...import *` 可以导入所有工具，但不推荐使用，不好排查不同模块函数重名的情况

5.模块的搜索顺序

- python的解释器在导入模块时，
  - 搜索**当前目录**指定模块名文件，**如果有就直接导入**
  - 如果没有，再搜索系统目录

> 在开发时，给文件起名，不要和**系统的模块文件重名**

python中每一个模块都有内置属性`__file__`可以查看模块的地址

6.**开发原则，每一个文件都应该是可以被导入的**

- 一个独立的python文件就是一个**模块**
- 在导入文件时，文件中**所有没有任何缩进的代码**都会被执行一遍

- `__name__`属性

> `__name__`属性可以做到，测试模块的代码只在测试情况下被运行，而在导入时不会被执行

- `__name__`是python的一个内置属性，记录着一个**字符串**
- 如果**是被其他文件导入的**，`__name__`就是模块名
- 如果是**当前执行程序**`__name__`就是`__main__`
...
- 要在外界使用包中的模块，需要在`__init__.py中指定`**对外界提供的模块列表**
